...
type = "wavelet"  # Choose between "wavelet" or "stft" or "integral" or "nothing"
...
fs = 313
overlaps = [0.6]  # Overlap percentage
# Define the wavelet families to use (only used if type == "wavelet")
...
labels_data_path = os.path.join(script_dir, test, folder1, 'labels2.npy')

# Define file paths for the second dataset
piezo_data_path_2 = os.path.join(script_dir, test, folder2, f'{finger}_cleaned.npy')
labels_data_path_2 = os.path.join(script_dir, test, folder2, 'labels2.npy')
...
if type not in ["nothing", "integral"]:
    segments = {
        wl: {
...
def train_and_evaluate_svm(wl, w, ov, C, gamma, kernel, X_train, y_train, X_test, y_test):
    ...
    # Return results based on the type
    return {
        "window_length": wl if type not in ["nothing", "integral"] else None,
        "wavelet": w if type == "wavelet" else None,
        "overlap": ov if type not in ["nothing", "integral"] else None,
        "C": C,
        "gamma": gamma,
        "kernel": kernel,
        "accuracy": accuracy,
...
def train_and_evaluate_hmm(wl, w, ov, X_train, y_train, X_test, y_test, unsupervised=True, initial_start_prob=None, initial_transition_matrix=None, n_states=2):
    ...
    - w: Wavelet type (used for logging)
...
if classifier == "svm":
    y_pred = best_result_test_on_set2["model"].predict(labeled_features2[:, :-1])
elif classifier == "hmm":
    if unsupervised:
        y_pred = best_result_test_on_set2["hidden_states_test"]
    else:
        y_pred = []
        for sample in labeled_features2[:, :-1]:
...
for label in np.unique(y_pred):
    plt.scatter(
        ...
        labeled_features2[y_pred == label, :-1].mean(axis=1),  # Y-axis: mean feature value (or any other feature)
        label=f"Predicted Class {label}",
        alpha=0.6,
        s=0.5
    )
plt.plot(label_points2, label="True Class", color="black", linestyle="--")
